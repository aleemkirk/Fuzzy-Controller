...
void loop() {
  ...
  flcOutput = (float)k3*FLC1.findslewingFLCoutput(k2*beta, k1*posErr);
  desAngle = (int)(filteredFB + (float)k3*FLC1.findslewingFLCoutput(k2*beta, k1*posErr));
  if ((int)flcOutput >= thresh || (int)flcOutput <= -1*thresh){
    s1.attach(servoPin);
    s1.write(desAngle);
  }else{
    s1.detach();
